...
function _h_render_icon_block( $atts, $default_atts ) {
  ...
  // format the content
  $icon_markup = $atts['iconMarkup'];
  $heading = $atts['heading'];
  $description = $atts['hasDescription'] ? '<dd>' . $atts['description'] . '</dd>' : '';

  $content = "<figure>$icon_markup</figure>
    <dl>
      <dt>$heading</dt>
      $description
    </dl>";
  ...
  $bg_color = $atts['bgColor'];
  $style = ( $text_color ? "--textColor: $text_color;" : '' ) .
  ...
  // format class name
  $class_name = 'wp-block-h-icon ' .
    ( $atts['className'] ?? '' ) .
    ' has-icon-position-' . $atts['iconPosition'] .
    ' has-text-align-' . $atts['align'] . ' ' .
    ( $bg_color === 'none' ? '' : 'has-background' ) . ' ' .
    ( $description ? 'has-description' : '' );

  return ( $url && $atts['isFullyClickable'] ) ?
    "<a class='$class_name' style='$style' href='$url' target='$target'>
      $content
    </a>"
  :
